...
fn main() {
    ...
    let duration_since:Duration = duration_since_result.clone().unwrap();
    println!("1 duration_since:{:?}", duration_since);
    ...
    let duration:Duration = duration_since_result.clone().unwrap_or(Duration::new(0u64,0u32));
